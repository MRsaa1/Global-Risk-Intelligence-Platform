#!/usr/bin/env bash
# ===========================================
# Тест API в режиме С GPU (NIM на сервере).
#
# Вариант A — запуск НА СЕРВЕРЕ (после SSH):
#   cd ~/global-risk-platform
#   ./scripts/gpu-test-gpu.sh
#   TOKEN='ваш-jwt' ./scripts/gpu-test-gpu.sh
#
# Вариант B — запуск НА МАКЕ через туннель (ssh -L 19002:localhost:9002 ...):
#   BASE_URL=http://127.0.0.1:19002 ./scripts/gpu-test-gpu.sh
#
# Результаты: docs/gpu-test-artifacts/gpu/
# ===========================================
...
if command -v jq >/dev/null 2>&1; then
  ...
  echo "  Для сводки запустите scripts/gpu-test-compare.sh"
fi
echo ""
