#!/usr/bin/env bash
# ===========================================
# Тест API в режиме БЕЗ GPU (локально / Contabo).
# Запуск: API должен быть доступен (по умолчанию http://127.0.0.1:9002).
#   ./scripts/gpu-test-local.sh
#   BASE_URL=http://localhost:9002 ./scripts/gpu-test-local.sh
# С JWT для stress test (в zsh обязательно в кавычках):
#   TOKEN='your-jwt-here' ./scripts/gpu-test-local.sh
# Результаты: docs/gpu-test-artifacts/local/
# ===========================================
...
cd "$(dirname "$0")/.."
ROOT="$(pwd)"
ARTIFACTS="$ROOT/docs/gpu-test-artifacts/local"
...
echo -e "${CYAN}=== Тест режима БЕЗ GPU (local) ===${NC}"
...
# 3) Опционально: stress test execute (если задан TOKEN)
if [ -n "${TOKEN}" ]; then
  ...
  echo -e "${YELLOW}3. POST /stress-tests/execute — пропущен (задайте TOKEN для вызова)${NC}"
fi

# Сводка
echo ""
echo -e "${CYAN}--- Сводка (ожидается без GPU) ---${NC}"
if command -v jq >/dev/null 2>&1; then
  NIM=$(jq -r '.nvidia_services.fourcastnet_nim.ready // empty' "$ARTIFACTS/health_nvidia.json" 2>/dev/null)
  if [ -z "$NIM" ] || [ "$NIM" = "null" ]; then
    NIM="false (не настроен)"
  fi
  ...
  echo "  Ожидание: false или unavailable"
else
  echo "  Файлы: $ARTIFACTS/health_nvidia.json, $ARTIFACTS/nim_health.json"
  echo "  Для сводки установите jq и запустите scripts/gpu-test-compare.sh"
fi
echo ""
