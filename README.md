...
---
>>>>>>> 03d91c7 (Docs: BRIEF_FOR_DEVELOPER, NVIDIA_APPLICATION_BRIEF (full list + code), DEPLOY_CONTABO; Security: .gitignore .env.nvidia and patterns)

**→ [http://127.0.0.1:5180/command](http://127.0.0.1:5180/command)** (local)
...
## Architecture: Five Layers of Reality
...
<<<<<<< HEAD
| **AI/ML** | PyTorch, PyG, **NVIDIA Cloud API** (Llama 3.1 — used when `NVIDIA_API_KEY` is set), AI-Q (RAG + citations), Generative AI endpoints. Optional: local NIM, Dynamo/Triton (Linux + NVIDIA GPU only) |
| **Speech** | **Web Speech API** (browser) for “Read aloud” — works without a server. Optional: NVIDIA Riva (Linux + NVIDIA GPU only; NIM images require paid subscription) |
=======
...
<<<<<<< HEAD
**Docs:** [docs/BREV_DEPLOYMENT.md](docs/BREV_DEPLOYMENT.md) | [docs/DEPLOY_CONTABO.md](docs/DEPLOY_CONTABO.md) | [QUICK_START.md](QUICK_START.md)

---

## Access (after local startup)

- **Command Center (main screen):** http://127.0.0.1:5180/command
- **Web UI (dashboard):** http://127.0.0.1:5180
=======
...
- **SRS, CityOS, FST:** on roadmap (Phase 1); SRS/CityOS — design and MVP in later years
...
### NVIDIA services (dashboard block and health API)

Matches `GET /api/v1/health/nvidia` and the **NVIDIA Services** block on the dashboard.

| Dashboard service | Default status | When it runs |
|-------------------|----------------|--------------|
| **NVIDIA LLM (Cloud API)** | OK when key set | `NVIDIA_API_KEY` — agents, reports, Generative AI (Llama 3.1). |
| **NVIDIA AI Orchestration** | OK when key set | Same key — multi-model consensus for stress tests. |
| **Earth-2 FourCastNet NIM** | disabled | `USE_LOCAL_NIM` + `fourcastnet_nim_url`, container on Linux + NVIDIA GPU. |
| **Earth-2 CorrDiff NIM** | disabled | `USE_LOCAL_NIM` + `corrdiff_nim_url`, container on Linux + NVIDIA GPU. |
| **FLUX.1-dev NIM** | by config/availability | `flux_nim_url` — image generation for REPORTER. |
| **NVIDIA Earth-2** | by config/availability | `earth2_api_url` — climate/weather in climate_data. |
| **PhysicsNeMo** | by config/availability | `physics_nemo_api_url` — physics simulation layer. |
| **NVIDIA Riva** | disabled | `ENABLE_RIVA=true` + deployed Riva (Linux + NVIDIA GPU). Voice for alerts and reports. |
| **NVIDIA Dynamo** | disabled | `ENABLE_DYNAMO=true` + deployed Dynamo (Linux + NVIDIA GPU). |
| **Triton Inference Server** | disabled | `ENABLE_TRITON=true` + deployed Triton (Linux + NVIDIA GPU). |
| **NeMo Retriever** | OK (by config) | `nemo_retriever_enabled` — RAG, citations in AI-Q. |
| **NeMo Guardrails** | OK (by config) | `nemo_guardrails_enabled` — checks before agent output. |
| **NeMo Agent Toolkit** | OK (by config) | `nemo_agent_toolkit_enabled` — agent monitoring. |
| **NeMo Curator** | OK (by config) | `nemo_curator_enabled` — data curation. |
| **NeMo Data Designer** | OK (by config) | `nemo_data_designer_enabled` — synthetic data. |
| **NeMo Evaluator** | OK (by config) | `nemo_evaluator_enabled` — agent evaluation. |

**Read aloud:** No server — **Web Speech API** in the browser (Mac, Windows, Linux). Optionally — Riva when enabled and deployed.

**Live status in UI:** The **NVIDIA Services** block in Command Center shows for each service: enabled/disabled, URL, Source, Call. “OK” for NIM/Earth-2/PhysicsNeMo — when availability check succeeds; for NeMo Retriever/Guardrails etc. — “enabled in config”.
...
### Navigation
- **[MASTER_PLAN.md](MASTER_PLAN.md)** — central document with full navigation
...
**Current state:** Command Center with 3D globe, stress tests, risk zones, Cascade, Digital Twin, agents (SENTINEL, ANALYST, ADVISOR, REPORTER), System Overseer, Strategic Modules (CIP, SCSS, SRO + registry), Generative AI (explanations, recommendations, disclosure draft, AI-Q chat). **NVIDIA in use:** Cloud API for LLM only (when `NVIDIA_API_KEY` is set); voice — Web Speech API in the browser. Riva, Dynamo, Triton — integration code present, disabled by default; require Linux and NVIDIA GPU (Riva NIM — paid subscription).

**Live:** [risk.saa-alliance.com](https://risk.saa-alliance.com) (Global Risk Intelligence Platform)
=======
**Status:** Active Development - Alpha  
**Last Updated:** January 2026  
