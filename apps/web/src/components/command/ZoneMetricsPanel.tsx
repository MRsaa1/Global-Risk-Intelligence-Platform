...
const orgTypes = [
  { id: 'enterprise', label: 'Enterprise', icon: BuildingOffice2Icon, color: 'text-zinc-400' },
  { id: 'bank', label: 'Banks', icon: BuildingLibraryIcon, color: 'text-zinc-400' },
  { id: 'insurer', label: 'Insurers', icon: ShieldCheckIcon, color: 'text-zinc-400' },
  { id: 'infrastructure', label: 'Infrastructure', icon: WrenchScrewdriverIcon, color: 'text-zinc-400' },
  { id: 'government', label: 'Government', icon: BuildingLibraryIcon, color: 'text-zinc-400' },
]
...
function getRiskColor(risk: number): string {
  if (risk >= 0.8) return 'text-red-400/80'
  if (risk >= 0.6) return 'text-orange-400/80'
  if (risk >= 0.4) return 'text-amber-400/80'
    ...
    <div className="h-full bg-black/80 border border-zinc-800 rounded-md p-4 flex flex-col">
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-sm font-medium text-zinc-100">Zone Details</h3>
        </div>
      ...
      {selectedZone && (
        <AnimatePresence mode="wait">
          {activeTab === 'metrics' && (
            ...
            >
              {/* Zone Name */}
              <div className="p-3 bg-zinc-800 rounded-md border border-zinc-800">
                <div className="font-mono text-[10px] uppercase tracking-widest text-zinc-500 mb-1">Zone</div>
                <div className="text-sm font-medium text-zinc-100">{selectedZone.name}</div>
                <div className="text-xs text-zinc-500 mt-0.5">Level: {selectedZone.zone_level}</div>
              </div>

              {/* Metrics Grid */}
              <div className="grid grid-cols-2 gap-2">
                {/* Risk Score */}
                <div className="p-3 bg-gradient-to-br from-red-500/10 to-transparent rounded-md border border-red-500/20">
                  <div className="flex items-center gap-1.5 mb-1">
                    <ExclamationTriangleIcon className="w-3.5 h-3.5 text-red-400/80" />
                    <span className="font-mono text-[10px] uppercase tracking-widest text-zinc-500">Risk Score</span>
                  </div>
                ...
                <div className="p-3 bg-zinc-800 rounded-md border border-zinc-700">
                  <div className="flex items-center gap-1.5 mb-1">
                    ...
                    <span className="font-mono text-[10px] uppercase tracking-widest text-zinc-500">Exposure</span>
                  </div>
                ...
                {/* Affected Assets */}
                <div className="p-3 bg-gradient-to-br from-zinc-700/30 to-transparent rounded-md border border-zinc-700">
                  <div className="flex items-center gap-1.5 mb-1">
                    <BuildingOffice2Icon className="w-3.5 h-3.5 text-zinc-400" />
                    <span className="font-mono text-[10px] uppercase tracking-widest text-zinc-500">Assets</span>
                  </div>
                ...
                <div className="p-3 bg-gradient-to-br from-zinc-700/30 to-transparent rounded-md border border-zinc-700">
                  <div className="flex items-center gap-1.5 mb-1">
                    ...
                    <span className="font-mono text-[10px] uppercase tracking-widest text-zinc-500">Recovery</span>
                  </div>
                  <div className="text-xl font-bold text-zinc-400">
              ...
              {/* Radius Info */}
              <div className="p-2 bg-zinc-800 rounded-md text-xs text-zinc-500 flex items-center justify-between">
          ...
          {activeTab === 'entities' && (
            ...
            >
              ...
              {orgTypes.map(orgType => {
                ...
                return (
                  <div key={orgType.id} className="bg-zinc-800 rounded-md border border-zinc-800 overflow-hidden">
                    {/* Group header */}
                    <div className="px-3 py-2 border-b border-zinc-800 flex items-center gap-2">
                      <Icon className={`w-4 h-4 ${orgType.color}`} />
                      <span className="text-xs font-medium text-zinc-100">{orgType.label}</span>
                      <span className="text-[10px] text-zinc-500 ml-auto">{typeEntities.length}</span>
                    </div>
                    ...
                    <div className="max-h-28 overflow-y-auto">
                      {typeEntities.map(entity => (
                        <div 
                          key={entity.id}
                          className="px-3 py-2 flex items-center justify-between border-b border-zinc-800 last:border-0 hover:bg-zinc-800 transition-colors"
                        >
                          <span className="text-xs text-zinc-200 truncate flex-1">{entity.name}</span>
                          <div className="flex items-center gap-3 text-[10px]">
                            <span className="text-zinc-500">{formatBillions(entity.exposure)}</span>
