  ...
  return (
    <group ref={groupRef} position={[0, -0.6, 0]}>
      ...
      <mesh rotation={[-Math.PI / 2, 0, 0]} position={[0, -0.01, 0]}>
        ...
        <meshStandardMaterial color="#1a1f2e" metalness={0.2} roughness={0.8} />
      </mesh>
    ...
    <div className="h-full flex flex-col bg-[#000510]">
      ...
      <div className="flex-1 flex">
        ...
        <div className="w-28 p-3 space-y-1.5 border-r border-[#1a2535]">
          ...
          <div className="pt-2">
            <div className="text-gray-500 text-[10px] mb-1">Risk:</div>
            <div className="flex gap-0.5">
              {[1, 2, 3, 4, 5].map((i) => (
                <div
                  key={i}
                  className={`h-2 flex-1 rounded-sm ${
                    i <= asset.riskLevel ? 'bg-red-500' : 'bg-[#1a2535]'
                  }`}
                />
              ))}
            </div>
        ...
        <div className="w-36 p-3 space-y-2 border-l border-[#1a2535]">
      ...
      <div className="px-4 py-2 border-t border-[#1a2535] bg-[#0a0f18]/90">
        <div className="flex items-center gap-3 text-xs">
          ...
          <ImpactItem label="Capital Adequacy" value={`${asset.impact.capitalAdequacy}%`} color="text-amber-500" icon="⚠️" />
...
function RiskRow({ 
  ...
  return (
    <div className="flex items-center justify-between text-[10px]">
      ...
      <span className={`flex items-center gap-1 ${status ? colors[status] : 'text-white'}`}>
        {value}
