  ...
  return (
    <div className="flex h-screen bg-[#0a0e17]">
      ...
      <aside className={`${isCommandCenter ? 'hidden' : 'w-20'} flex flex-col items-center py-4 border-r border-[#1a2535] bg-[#0a0f18] transition-all`}>
        ...
        <div className="mb-6">
          <motion.div
            className={`${isCommandCenter ? 'w-10 h-10' : 'w-12 h-12'} rounded-xl bg-gradient-to-br from-cyan-500 to-blue-600 flex items-center justify-center cursor-pointer`}
        ...
        <div className="mt-auto flex flex-col gap-2">
          <div className={`${isCommandCenter ? 'w-8 h-8 text-xs' : 'w-10 h-10 text-sm'} rounded-full bg-gradient-to-br from-cyan-400 to-blue-500 flex items-center justify-center text-white font-semibold`}>
            A
          </div>
          <button
      ...
      <main className={`flex-1 relative ${isCommandCenter ? 'overflow-hidden' : 'overflow-auto'}`}>
        <Outlet />
        {!isCommandCenter && <FeedbackButton />}
      </main>
    </div>
  )
}
