...
Idempotent: skips create_table if table already exists (e.g. after partial run on SQLite).
"""
...
def upgrade() -> None:
    ...
    if not _has_table("raw_source_records"):
        op.create_table(
            "raw_source_records",
    ...
    if not _has_table("normalized_events"):
        op.create_table(
            "normalized_events",
            sa.Column("id", sa.Integer(), primary_key=True, autoincrement=True),
            sa.Column("source_name", sa.String(64), nullable=False),
    ...
    if not _has_table("event_entities"):
        op.create_table(
            "event_entities",
    ...
    # 4) Event losses
    if not _has_table("event_losses"):
        op.create_table(
            "event_losses",
            sa.Column("id", sa.Integer(), primary_key=True, autoincrement=True),
    ...
    # 5) Event impacts
    if not _has_table("event_impacts"):
        op.create_table(
            "event_impacts",
            sa.Column("id", sa.Integer(), primary_key=True, autoincrement=True),
            sa.Column("event_uid", sa.String(36), sa.ForeignKey("event_entities.event_uid"), nullable=False),
    ...
    # 6) Event recovery
    if not _has_table("event_recovery"):
        op.create_table(
            "event_recovery",
            sa.Column("id", sa.Integer(), primary_key=True, autoincrement=True),
            sa.Column("event_uid", sa.String(36), sa.ForeignKey("event_entities.event_uid"), nullable=False),
    ...
    # 7) Source registry
    if not _has_table("source_registry"):
        op.create_table(
            "source_registry",
    ...
    if not _has_table("data_quality_scores"):
        op.create_table(
            "data_quality_scores",
            sa.Column("id", sa.Integer(), primary_key=True, autoincrement=True),
